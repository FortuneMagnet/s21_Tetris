...
void dropFigure(GameInfo_t* t) {
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
      if (t->figure[i][j] == 2) t->field[i + t->y][j + t->x] = t->figure[i][j];
    }
  }
}
...
void deleteFigure(GameInfo_t* t) {
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
      if (t->figure[i][j] == 2) t->field[i + t->y][j + t->x] = 0;
    }
  }
}
...
void plantFigure(GameInfo_t* t) {
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
      if (t->figure[i][j] == 2) t->field[i + t->y][j + t->x] = 1;
    }
  }
}

void moveFigureDown(GameInfo_t* t) { t->y++; }

void moveFigureUp(GameInfo_t* t) { t->y--; }

void moveFigureRight(GameInfo_t* t) { t->x++; }

void moveFigureLeft(GameInfo_t* t) { t->x--; }
...
void dropLine(int i, GameInfo_t* t) {
  if (i == 0) {
    for (int j = 0; j < cols; j++) t->field[i][j] = 0;
  } else {
    for (int k = i; k > 0; k--) {
...
void levelProcessing(GameInfo_t* t) {
  //Добавь в игру механику уровней. Каждый раз, когда игрок набирает 600 очков,
  //уровень увеличивается на 1. Повышается скорость, меняется тикрейт
...
GameInfo_t updateCurrentState(GameInfo_t* t) {
  ...
  if (t->ticks == 0) {
    ...
    if (collisionTet(t)) {
      ...
      levelProcessing(t);
      dropNewFigure(t);
      if (collisionTet(t)) t->state = GAME_OVER;  /// state
    }
  ...
  if (t->action == Left) {
    moveFigureLeft(t);
    if (collisionTet(t)) moveFigureRight(t);
  }
  if (t->action == Right) {
    moveFigureRight(t);
    if (collisionTet(t)) moveFigureLeft(t);
  }
  if (t->action == Up) {
    rotateFigure(t);
    if (collisionTet(t)) {
