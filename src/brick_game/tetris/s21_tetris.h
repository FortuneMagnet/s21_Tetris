 ...
 *
 *  This function sets the initial position of the new figure, copies the next figure to the current figure,
 *  and chooses a new random figure for the next one.
 * 
 * \param t Pointer to the GameInfo_t structure.
 */
 ...
 *
 *  If the current score is higher, updates the high score in the "hight_score.txt" file.
 * 
 * \param t Pointer to the GameInfo_t structure.
 */
 ...
 *
 *  Places the current figure onto the game field, marking its cells with the figure value (2).
 * 
 * \param t Pointer to the GameInfo_t structure.
 */
 ...
 *
 *  Clears the cells on the game field that were occupied by the current figure, setting them to 0.
 * 
 * \param t Pointer to the GameInfo_t structure.
 */
 ...
 *
 *  Places the current figure onto the game field, marking its cells with the planted figure value (1).
 * 
 * \param t Pointer to the GameInfo_t structure.
 */
...
/*!
 * \brief Checks for collisions between the current figure and the game field or boundaries.
 *
 ...
 *
 *  Shifts all lines above the specified line down one cell, effectively deleting the specified line.
 * 
 ...
 *
 *  Iterates through the game field, erases complete lines, drops lines above them,
 *  and updates the score based on the number of lines erased.
 * 
 * \param t Pointer to the GameInfo_t structure.
 ...
 *
 * This function allocates memory for a new `GameInfo_t` structure, initializes the game field, 
 * the next figure, and the current figure, and sets the initial game state to START.
 * 
 ...
 *
 * This function frees the memory of the current game state, creates a new game state,
 ...
 *
 * This function allocates memory for the game field and initializes all cells to 0.
 * 
 * \param t Pointer to the `GameInfo_t` structure.
 * \return A pointer to the `GameInfo_t` structure with the initialized game field.
 */
 ...
 *
 * This function allocates memory for the next figure and chooses a random shape for it.
 *
 * \param t Pointer to the `GameInfo_t` structure.
 * \return A pointer to the `GameInfo_t` structure with the initialized next figure.
 */
 ...
 *
 * This function allocates memory for the current figure, initializes it to empty,
 * and drops a new figure onto the game field.
 * 
 * \param t Pointer to the `GameInfo_t` structure.
 * \return A pointer to the `GameInfo_t` structure with the initialized current figure.
 */
 ...
 *
 * This function allocates memory for a 4x4 temporary array used for figure rotation.
 * 
 ...
 *
 * This function frees all memory allocated for the game state, including the game field, 
 * the next figure, and the current figure.
 *
 * \param t Pointer to the `GameInfo_t` structure.
 */
 ...
 *
 * This function processes user input (key presses) and sets the appropriate action in the `GameInfo_t` structure.
 *
 * \param t Pointer to the `GameInfo_t` structure.
 ...
 * 
 * This function initializes the ncurses library, sets up color pairs, and configures
 * basic terminal settings for the game.
 */
void init_ncurses();
 ...
 *
 * This function draws the game field using ncurses, showing the current state of the field 
 * with bricks and empty spaces.
 * 
 * \param t Pointer to the `GameInfo_t` structure.
 */
 ...
 *
 * This function uses ncurses to display the current score, high score, and level 
 * in designated areas on the screen.
 *
 * \param t Pointer to the `GameInfo_t` structure.
 */
 ...
 *
 * This function displays a message box with appropriate text for pause or game over states,
 * using ncurses for visual presentation.
 * 
 * \param t Pointer to the `GameInfo_t` structure.
 */
 ...
 *
 * This function displays a welcome message and instructions for starting the game
 * using ncurses.
 */
 ...
 * 
 * This function calls other drawing functions to draw the game field, next figure, score,
 * and other relevant information for the current game state.
 *
 * \param t Pointer to the `GameInfo_t` structure.
 */
